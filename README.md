...
| held to a trials budget | `N ≤ 5e5` forces a priority order (model-motivated axes first, then highest expected rate): 5842 of 36906 spectra survive, all of them on the 41 model-motivated compositions, `Z_local = 7.16` | [`scaled_scan.py`](scripts/scaled_scan.py) | [`priority_scan.csv`](results/tables/priority_scan.csv), per composition kept against dropped | [`scaled_scan.py`](scripts/scaled_scan.py) | [`scaled_scan.csv`](results/tables/scaled_scan.csv) with one row per variant, [`scaled_scan.txt`](results/tables/scaled_scan.txt) |
