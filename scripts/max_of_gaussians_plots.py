...
from plot_style import SURF, INK, INK2, GRID, BLUE, C_ARG, C_THR, C_ALT, panels, title, labels
...
fig, (a0, a1) = panels(figsize=(13.4, 5.0), wspace=0.20)
...
a1.plot(xs, pdf_max(xs), color=BLUE[5], lw=2.4, label="exact:  $n\\,\\phi(x)\\,\\Phi(x)^{n-1}$")
a1.plot(xs, g.pdf(xs), color=C_ALT, lw=2.2, ls=(0, (5, 2.5)),
...
fig, (b0, b1) = panels(figsize=(13.4, 5.0), wspace=0.20)

xs = np.linspace(1.0, 8.5, 900)
b0.fill_between(xs, nb * pdf(xs) * cdf(xs) ** (nb - 1), color="#c9c8c4", lw=0, alpha=0.85)
b0.plot(xs, nb * pdf(xs) * cdf(xs) ** (nb - 1), color=INK2, lw=1.6)
for m in MUS:
...
b1.plot(mus, pw * 100, color=BLUE[5], lw=2.6)
...
fig.savefig(os.path.join(PLOTS, "signal_wins_the_max.png"), dpi=170, facecolor=SURF,
            bbox_inches="tight")

# ---------------------------------------------------------------- FIGURE 3: A/B confirmation
fig, (c0, c1) = panels(figsize=(13.4, 5.0), wspace=0.20)
...
for i, (u, v) in enumerate(zip(s1, s2)):
    c0.text(i - w / 2, u + 2.5, f"{u:.0f}%", ha="center", color=INK, fontsize=12)
    c0.text(i + w / 2, v + 2.5, f"{v:.0f}%", ha="center", color=BLUE[5], fontsize=12)
c0.set_xticks(xpos)
c0.set_xticklabels([f"${m}\\sigma$" for m in MUS], fontsize=13)
c0.set_ylim(0, 132)
c0.legend(frameon=False, fontsize=11, labelcolor=INK2, loc="upper left")
labels(c0, "signal strength  $\\mu$", "probability  [%]")
title(c0, "Two-stage A/B confirmation")

mus = np.linspace(1.5, 8.5, 100)
c1.plot(mus, [p_win(m) * 100 for m in mus], color=BLUE[3], lw=2.4)
c1.plot(mus, [y_arg(m) * 100 for m in mus], color=BLUE[5], lw=2.6)
for m in MUS:
    v = y_arg(m) * 100
...
fig, (d0, d1) = panels(figsize=(13.4, 5.0), wspace=0.22)

ts = np.linspace(3.0, 6.0, 600)
d0.semilogy(ts, lam(ts), color=C_THR, lw=2.6)
d0.axhline(1, color=INK2, lw=1, ls=(0, (4, 3)))
d0.axvline(tstar, color=INK2, lw=1, ls=(0, (2, 3)))
d0.plot([tstar], [1], "o", ms=9, color=C_THR, mec=SURF, mew=2, zorder=6)
...
for m in MUS:
    ...
    d1.plot([m], [50], "o", ms=7, color=BLUE[m], mec=SURF, mew=1.8, zorder=6)
    d1.text(m - 0.07, 54, f"$\\mu={m}\\sigma$", color=BLUE[m], fontsize=11.5, ha="right",
            fontweight="medium")
...
fig, (e0, e1) = panels(figsize=(13.4, 5.0), wspace=0.24)

for m in MUS:
    e0.plot(ts, y_thr(m, ts) * 100, color=BLUE[m], lw=2.4)
    pa = y_arg(m) * 100
    e0.plot([3.0, 6.0], [pa, pa], color=BLUE[m], lw=1.2, ls=(0, (2, 2.5)), alpha=0.85)
    e0.text(6.05, pa, f"${m}\\sigma$", color=BLUE[m], fontsize=11.5, va="center",
            fontweight="medium")
...
# ---------------------------------------------------------------- FIGURE 6: the ROC
fig, (f0, f1) = panels(figsize=(14.2, 5.3), wspace=0.24, width_ratios=[1.12, 1])

tr = np.linspace(2.6, 6.6, 800)
f0.axvline(x_arg, color=C_ARG, lw=1.3, ls=(0, (3, 3)), alpha=0.9)
for m in MUS:
    f0.semilogx(x_thr(tr), y_thr(m, tr) * 100, color=BLUE[m], lw=2.4)
    f0.plot([x_arg], [y_arg(m) * 100], marker="D", ms=9, color=C_ARG, mec=SURF, mew=1.8, zorder=6)
    f0.plot([x_thr(tstar)], [y_thr(m, tstar) * 100], "o", ms=8, color=BLUE[m], mec=SURF, mew=1.8,
            zorder=6)
    f0.text(0.42, {3: 32, 4: 76, 5: 93, 6: 106}[m], f"$\\mu={m}\\sigma$", color=BLUE[m],
            fontsize=11.5, va="center", fontweight="medium")
for tt in (3, 5, 6):
    f0.plot([x_thr(tt)], [y_thr(5, tt) * 100], marker="|", ms=11, color=BLUE[5], mew=2, zorder=5)
    f0.text(x_thr(tt), y_thr(5, tt) * 100 - 8, f"$t={tt}$", color=BLUE[5], fontsize=10,
            ha="center")
...
for m in MUS:
    ...
    f1.plot([tstar], [y_thr(m, tstar) / x_thr(tstar)], "o", ms=7, color=BLUE[m], mec=SURF,
            mew=1.6, zorder=6)
    f1.text(6.05, y_thr(m, 6.0) / x_thr(6.0), f"${m}\\sigma$", color=BLUE[m], fontsize=11.5,
