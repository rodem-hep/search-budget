...
| fully combinatorial scan | 712 fittable mass spectra of 1502 across 204 object categories, giving `N ≈ 35k` and `Z_local ≈ 6.78` | [`combinatorial_budget.py`](scripts/combinatorial_budget.py) | [`combinatorial_budget.csv`](results/tables/combinatorial_budget.csv) |
| scaled up to ten object types | adding hadronic taus, photons, boosted W/Z, H and top: 3603 fittable spectra of 21644, `N ≈ 1.6e5`, `Z_local ≈ 7.00` | [`scaled_scan.py`](scripts/scaled_scan.py) | [`scaled_scan.csv`](results/tables/scaled_scan.csv), [`scaled_scan.txt`](results/tables/scaled_scan.txt) |
| only fittable histograms count | ≥100 events and ≥25 resolution elements holding ≥1 event, on a declared background-yield model calibrated per object type: it truncates every window at its one-event mass and drops 18041 of the 21644 histograms. A ×10⁴ band on the yield anchor moves `Z_local` by 0.14σ | [`yield_model.py`](scripts/yield_model.py) | run it for the per-object calibration table |
| which dataset that assumes | Run 2, 140 fb⁻¹, the dataset the published scan windows come from. Run 2 plus Run 3 is ×3 and makes 4438 spectra fittable rather than 3603, `Z_local` 7.03: reach grows only as the sixth root of the luminosity | [`scaled_scan.py`](scripts/scaled_scan.py) | [`scaled_scan.txt`](results/tables/scaled_scan.txt), the closing block |
| held to a trials budget | `N ≤ 5e5` never binds: the whole fittable scan costs 1.6e5, and 2.9e5 with lenses, so the priority order (model-motivated axes first, then highest yield) has nothing to cut. Statistics limits a wide scan, not the trials factor | [`scaled_scan.py`](scripts/scaled_scan.py) | [`priority_scan.csv`](results/tables/priority_scan.csv), per composition kept against dropped |
| selection lenses | HT/Meff, displaced, VBF and ISR views of the same axes, one lens at a time and each paying its own efficiency: 3008 views survive and 4686 are too thin to fit, giving 1.8 histograms per spectrum and `Z_local = 7.08` | [`scaled_scan.py`](scripts/scaled_scan.py) | [`lens_scan.csv`](results/tables/lens_scan.csv), per lens what it adds and what survives |
...
│   ├── yield_model.py       ** what a hypothetical spectrum holds, and whether it can be fitted **
