...
| scaled up to ten object types | adding hadronic taus, photons and boosted W/Z, H and top: 18580 spectra, `N ≈ 1.2M` and `Z_local ≈ 7.28`, where a local 5σ has no global significance left | [`scaled_scan.py`](scripts/scaled_scan.py) | [`scaled_scan.csv`](results/tables/scaled_scan.csv) with one row per variant, [`scaled_scan.txt`](results/tables/scaled_scan.txt) |
