...
def main(options=None):
    ...
    with open(io.ensure(paths.tex("uncertainty_table.tex")), "w") as f:
        ...
        f.write("\\begin{tabular}{@{}l>{\\raggedright\\arraybackslash}p{0.30\\textwidth}ccc@{}}\n"
                "\\toprule\n")
        f.write(f"source & varied over & {len(AXES)} spectra & scan & difference \\\\\n\\midrule\n")
        for s in SOURCES:
...
{per_src['mass resolution, scale'][0][0]:+.2f}: the errors average down over {len(AXES)} spectra, so the
