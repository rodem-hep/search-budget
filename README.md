...
| the two-body grid | 11 pairs with no catalogue axis cost 505 looks, from 30 (`tauH`) to 71 (`jV`); closing all of them takes `N` 4,118 → 4,623 and `Z_local` 6.45 → 6.47 | `two_body_matrix.csv` |
| two-body axes no published search scans | 7 axes over 7 pairs (`eb`, `mub`, `tauga`, `jb`, `bga`, `tt`, `tga`), 345 looks already inside the model space's `N` | `two_body_matrix.csv` |
...
| the axis set | non-peaking axes and the dilepton overlap dropped; the 11 two-body pairs with no catalogue axis added | +0.018/-0.004 | +0.000/+0.000 | +0.004/-0.018 |
