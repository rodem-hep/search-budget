...
@stage(
    name="published-census",
    group="census",
    summary="the ATLAS resonance-search publication record, by family and by recency",
    outputs=["tables/published_census.csv", "overviews/PUBLISHED_CENSUS.md"],
...
def main(options=None):
    ...
    for fam in sorted(by_fam):
        ...
        table.append([fam, len(sel), pap_fam[fam]]
                     + [sum(1 for r in sel if r["status"] == s) for s in STATUS]
                     + [sum(1 for r in sel if r["run3"] == "yes")])
    table.append(["TOTAL", len(rows), len(papers)]
                 + [by_status[s] for s in STATUS] + [len(run3)])
    io.write_rows(paths.table("published_census.csv"),
                  ["family", "spectra", "papers", "current", "ageing", "stale", "run3"], table)
...
({n_axes} canonical mass axes); this counts the searches that have actually been published
...
whole cost is analysis effort. The remaining {len(off_axis)} fall on no axis in the budget's {n_axes}, so re-running
...
"""
    io.write_text(paths.overview("PUBLISHED_CENSUS.md"), md)
