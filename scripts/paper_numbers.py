...
from bump_observables import res, SCAN
...
def load(name):
    with open(os.path.join(TAB, name)) as fh:
        return list(csv.DictReader(fh))


def z5(N):
    return math.sqrt(25.0 + 2.0 * math.log(N))


def zg(N):
    x = 25.0 - 2.0 * math.log(N)
    ...
    for a, segs in seg.items():
        ...
        for lo, hi in sorted(segs):
            if merged and lo <= merged[-1][1]:
                ...
                merged.append((lo, hi))
        tot += sum(math.log(hi / lo) / res(a) for lo, hi in merged)
...
body = ("\n\n".join(blocks) + "\n\nEvery row above is written by "
        "[`paper_numbers.py`](scripts/paper_numbers.py) from the committed tables, so `make all` "
        "keeps it and the paper in step; nothing here is typed by hand.")

readme = os.path.join(ROOT, "README.md")
text = open(readme).read()
i, j = text.index(START), text.index(END)
open(readme, "w").write(text[:i] + START + "\n\n" + body + "\n\n" + text[j:])
