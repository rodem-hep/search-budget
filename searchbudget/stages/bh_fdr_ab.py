...
from ..stats.fdr import accepted_grid, smallest_uniforms as _smallest_uniforms
from ..viz.style import (BLUE, C_ARG as c_arg, C_ARG as c_fake, C_BKG as c_bkg, C_THR as c_thr,
                         GRID as grid, INK as ink, INK2 as ink2, SURF as surf, style)
...
@stage(
    name="bh-fdr-ab",
    group="stats",
    summary="Benjamini-Hochberg against argmax at a matched false-alarm rate",
    outputs=["plots/max_of_gaussians/bh_scan.png", "plots/max_of_gaussians/bh_vs_argmax.png",
             "plots/max_of_gaussians/roc_bh_vs_threshold.png", "tables/bh_fdr_scan.csv"],
    caches=["tables/bh_fdr_mc.npz"],
)
def main(options=None):
    ...
    def bh_counts(p_sorted, qgrid):
        return accepted_grid(p_sorted, n, qgrid)
    ...
    CACHE = io.ensure(paths.table("bh_fdr_mc.npz"))
    ...
    io.write_rows(
        paths.table("bh_fdr_scan.csv"),
        ["q", "EK_null", "P_reject_null", "E_false_conf", "P_false_conf"]
        + [f"P_sel_mu{mu}" for mu in MUS] + [f"power_mu{mu}" for mu in MUS],
        [[f"{q:.4f}", f"{EK0[i]:.4f}", f"{P1_0[i]:.4f}", f"{x_bh[i]:.3e}", f"{PF0[i]:.3e}"]
         + [f"{SEL[mu][i]:.4f}" for mu in MUS]
    ...
    a1.set_title("Chance the signal bin passes BH",
                 color=ink, pad=12, loc="left")
    io.save(fig, paths.gaussians("bh_scan.png"), dpi=400, facecolor=surf, bbox_inches="tight")
    ...
    b1.set_title("Global false-alarm rate it buys", color=ink,
                 pad=12, loc="left")
    io.save(fig, paths.gaussians("bh_vs_argmax.png"), dpi=400, facecolor=surf, bbox_inches="tight")
    ...
    c1.set_title("Realized against nominal FDR", color=ink,
                 pad=12, loc="left")
    io.save(fig, paths.gaussians("roc_bh_vs_threshold.png"), dpi=400, facecolor=surf,
                bbox_inches="tight")
