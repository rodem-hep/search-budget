...
if N_model:
    print(f"model side (reference): N = {N_model:,.0f} over {len(model)} axes, Z_local = {z5(N_model):.2f}")
...
with open(_p("results", "tables", "census_budget.csv"), "w", newline="") as f:
    w = csv.writer(f)
    w.writerow(["family", "spectrum", "status", "budget_axis", "window_GeV", "window_from",
                "r", "n_s"])
    for row, axis, r, segs, ns in entries:
        ...
        w.writerow([row["family"], row["spectrum"], row["status"], axis or OFF,
...
def ns_union(a): return sum(n_s(lo, hi, res(a)) for lo, hi in union[a])
...
def md_axes():
    ...
    for a in sorted(union, key=lambda a: -ns_union(a)):
        ...
        lines.append(f"| `{a}` | {segstr(union[a])} | {res(a):g} | {ns_union(a):.0f} | {ref} |")
    return "\n".join(lines)
...
def md_levels():
    ...
    rows_ = [(f"**published searches** ({len(rows)} census entries)", len(priced), N_entry),
             (f"axes scanned (union of the ranges)", len(covered) + len(off_axis), N_axes)]
    if N_model:
        rows_.append((f"model-motivated axes (reference, `SEARCH_BUDGET.md`)", len(model), N_model))
...
"""
open(_p("results", "overviews", "CENSUS_BUDGET.md"), "w").write(md)
