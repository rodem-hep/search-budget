...
| scaled up to ten object types | adding hadronic taus, photons and boosted W/Z, H and top, with the four-object limit on the mass combination rather than the event: 21644 spectra, `N ≈ 1.5M` and `Z_local ≈ 7.31` at four objects per category, rising to 8.10 at eight, past the point (`N ≈ 2.7e5`) where a local 5σ has any global significance left | [`scaled_scan.py`](scripts/scaled_scan.py) | [`scaled_scan.csv`](results/tables/scaled_scan.csv) with one row per variant, [`scaled_scan.txt`](results/tables/scaled_scan.txt) |
