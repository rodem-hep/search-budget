...
from ..core.bump_observables import (SQRTS, floor, ns_achievable, ns_scan, res, scan_segments,
                                     scan_source, z_local_for_global5 as z5)
from ..core.catalogue import canonical_order, models_by_spectrum
from ..core.lee import band
from ..core.public_obs_map import NSEL, NSEL_DEFAULT, PUBLIC_OBS, WIDTH, nonpeak_only, nsel
...
@stage(
    name="search-budget",
    group="budget",
    summary="the model-space budget: N, Z_local and the per-spectrum table",
    outputs=["tables/search_budget.csv", "tables/search_budget_selections.csv",
             "overviews/SEARCH_BUDGET.md"],
)
def main(options=None):
    ...
    def segs(o):
        return "+".join(f"{lo:g}-{hi:g}" for lo, hi in scan_segments(o))

    io.write_rows(
        paths.table("search_budget.csv"),
        ["observable", "r", "floor_GeV", "scan_window_GeV", "ns_scan", "ns_envelope",
         "n_models_public", "n_event_selections", "ns_with_selections", "scan_source"],
        [[o, res(o), f"{floor(o):g}", segs(o), f"{incl[o]:.1f}", f"{env[o]:.1f}",
          len(pub_models[o]), nsel(o), f"{sel[o]:.1f}", scan_source(o)] for o in order])

    io.write_rows(
        paths.table("search_budget_selections.csv"),
        ["observable", "n_event_selections", "ns_inclusive", "ns_with_selections", "channels"],
        [[o, nsel(o), f"{incl[o]:.1f}", f"{sel[o]:.1f}",
          NSEL.get(o, (NSEL_DEFAULT, "default"))[1]] for o in order])
    ...
    def md_spectra():
        ...
        for n, o in enumerate(order, 1):
            lines.append(f"| {n} | `{o}` | {res(o):g} | "
                         f"{' + '.join(f'{lo:g}-{hi:g}' for lo, hi in scan_segments(o))} | "
                         f"{incl[o]:.0f} | {env[o]:.0f} | {len(pub_models[o])} | {nsel(o)} | "
                         f"{sel[o]:.0f} |")
    ...
    def md_levels():
        lines = ["| granularity | # spectra | N_trials | band (r x0.5..x2) | Z_local for 5s global "
                 "| band |", "|---|--:|--:|---|--:|---|"]
...
"""
    io.write_text(paths.overview("SEARCH_BUDGET.md"), md)
