...
|---|---|---|---|
| spectra the public model space populates | **63** | [`search_budget.py`](searchbudget/stages/search_budget.py) | [`search_budget.csv`](results/tables/search_budget.csv), one row per spectrum with its window and source |
| the trials factor and the discovery bar | `N = 4,319` → `Z_local = 6.46 +0.18/−0.16`, or `7,231` → `6.54` over the 111 published event selections | [`search_budget.py`](searchbudget/stages/search_budget.py) | [`SEARCH_BUDGET.md`](results/overviews/SEARCH_BUDGET.md) |
...
| what that scan adds that nobody has scanned | **51** of the 4438: a final state some public model predicts, with the resonance on that sub-system of it, on an axis no published ATLAS search scans; 2,329 looks over 18 axes | [`unscanned_spectra.py`](searchbudget/stages/unscanned_spectra.py) | [`new_spectra.csv`](results/tables/new_spectra.csv), one row per mass and category |
| the published record | **104** charged (search, axis) pairs, from **86** catalogued entries over **290** ATLAS papers on 64 distinct bump observables; 19 entries not revisited since before 2019, 6 carrying a published Run-3 result | [`published_census.py`](searchbudget/stages/published_census.py) | [`published_spectra.csv`](data/published_spectra.csv), one row per entry with its arXiv references |
| what the published record costs | the same census priced in trials: **`N = 7,875` → `Z_local = 6.55`** over those 104 pairs, or `N = 3,837` → `6.44` counting each axis once over the union of its published ranges, within 11% of the model space — the model side, reached from the publication record instead | [`census_budget.py`](searchbudget/stages/census_budget.py) | [`CENSUS_BUDGET.md`](results/overviews/CENSUS_BUDGET.md), per published search and per axis |
...
A factor 46 in `N` separates the published record from a combinatorial scan with its selection
...
|---|---|---|
| spectra the public model space populates | **63** | `search_budget.csv` |
| the model space, published windows | `N = 4,319` → `Z_local = 6.46` +0.18/-0.16 | `search_budget.csv`, `budget_uncertainty.csv` |
| the same at published event-selection granularity | 111 channels, `N = 7,231` → `6.54` | `search_budget_selections.csv` |
...
| the whole ladder | a factor 84 in `N`, worth +0.65σ | derived |
| model space → combinatorial scan | +0.57σ, +0.12/-0.23 as a difference | `budget_uncertainty.csv` |
...
| what the five sharpest axes carry | 34% of `N` | `search_budget.csv` |
| dropping the largest contributor | `Z_local` 6.46 → 6.45 | `search_budget.csv` |
| event selections instead of inclusive spectra | 111 channels, `6.54` | `search_budget_selections.csv` |
| the spectra the most model classes point at | `m(VV)` (13), `m(jj)` (11), `m(tt)` (9) | `model_spectrum_map.csv` |
| where the model classes come from | 43 from the FeynRules/UFO database, 15 from the literature sweep, referenced class by class | `model_classes.csv` |
| model-motivated spectra with no ATLAS scan the scan can fit | **19** in the scan's (category, mass-group) units, the pair-produced axis resolved into its legs; 21 counting the ones it cannot fit | `unscanned_spectra.csv` |
| **what the scan actually adds**: predicted spectra ATLAS has not scanned | **51** of the 4438 fittable spectra (1.1%), over 18 axes, listed one by one | `new_spectra.csv` |
| ... for scale, predicted at all (the category one a model produces, the mass its resonant sub-system) | 164 of them (3.7%), over 59 axes; the rest is territory no model points at | `unscanned_scan_units.csv` |
...
| priced once per axis, over the union of its ranges | `N = 3,837` → `6.44`, within 11.2% of the model space | `census_budget.csv` |
...
| tier (i), the final states models predict | 164 spectra, `N = 9,178` → `6.58` | `scan_summary.csv` |
| tier (ii), those masses, in categories nothing predicts | 2,944 spectra, `N = 143,036` | `scan_summary.csv` |
| tier (iii), no model points at the composition | 1,330 spectra, `N = 48,922` | `scan_summary.csv` |
...
| the same requirement applied to published axes | would keep 49 of 59 axes and 3,306 of 4,207 looks, so exempting them is conservative | `scan_summary.csv` |
| the two-body grid | 4 pairs with no catalogue axis cost 151 looks, from 30 (`tauH`) to 52 (`gaH`); closing all of them takes `N` 4,319 → 4,470 and `Z_local` 6.46 → 6.47 | `two_body_matrix.csv` |
| two-body axes no published search scans | 14 axes over 14 pairs (`eb`, `et`, `eH`, `mub`, `mut`, `muH`, `tauga`, `tauV`, `jb`, `jV`, `jH`, `bga`, `tt`, `tga`), 546 looks already inside the model space's `N` | `two_body_matrix.csv` |
...
| toy validation of the reach | single stage on the full dataset 6.51 against 6.54 analytic; optimised split 7.10 against 7.10 analytic; naive 50/50 split 7.76 against 7.73 analytic (on the model space, `N = 7,231`) | `ab_split_toys.csv` |
...
|---|---|--:|--:|--:|
| mass resolution, scale | every r x2 either way | +0.106/-0.108 | +0.133/-0.191 | +0.026/-0.083 |
| mass resolution, per channel | each r independently, x2 per sigma (16-84%) | +0.022/-0.020 | +0.000/+0.000 | +0.020/-0.022 |
...
| scan windows | every edge x1.4 either way (published), x1.25 (generic) | +0.032/-0.039 | +0.023/-0.024 | +0.016/-0.009 |
...
| the axis set | non-peaking axes and the dilepton overlap dropped; the 4 two-body pairs with no catalogue axis added | +0.005/-0.004 | +0.000/+0.000 | +0.004/-0.005 |
| the definition of one look | N x0.5 to x Z/sqrt(2 pi) | +0.145/-0.108 | +0.145/-0.099 | +0.009/+0.000 |
| the closed-form LEE relation | exact Gaussian-tail solution instead | +0.000/-0.037 | +0.000/-0.045 | +0.000/-0.008 |
| total | quadrature over the systematic sources above | +0.183/-0.162 | +0.231/-0.308 | +0.123/-0.230 |
