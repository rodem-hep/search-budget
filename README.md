...
| the model space, published windows | `N = 3,685` → `Z_local = 6.44` +0.18/-0.16 | `search_budget.csv`, `budget_uncertainty.csv` |
| the same at published event-selection granularity | 94 channels, `N = 6,597` → `6.53` | `search_budget_selections.csv` |
| the published ATLAS program | 100 charged (search, axis) pairs, `N = 7,710` → `6.55` | `census_budget.csv` |
...
| published record → lensed scan | a factor 47 in `N`, worth +0.56σ | derived |
| the whole ladder | a factor 98 in `N`, worth +0.68σ | derived |
| model space → combinatorial scan | +0.59σ, +0.12/-0.23 as a difference | `budget_uncertainty.csv` |
...
| ... drawn per channel instead of in common | +0.03/-0.02 | `budget_uncertainty.csv` |
...
| what the five sharpest axes carry | 40% of `N` | `search_budget.csv` |
| dropping the largest contributor | `Z_local` 6.44 → 6.42 | `search_budget.csv` |
| event selections instead of inclusive spectra | 94 channels, `6.53` | `search_budget_selections.csv` |
| the spectra the most model classes point at | `m(VV)` (12), `m(tt)` (9), `m(jj)` (8) | `model_spectrum_map.csv` |

### The published ATLAS program (Section 4)

| quantity | value | reproduced by |
|---|---|---|
| the record | **290** papers (2010–2026) in **86** catalogued searches over 62 bump observables | `published_census.csv`, `census_budget.csv` |
| (search, axis) pairs | 112, of which **100** carry a chargeable range, over 74 of the 86 searches | `census_budget.csv` |
| pairs charged a single look | 4 (a fixed mass, not a scan) | `census_budget.csv` |
| pairs with nothing chargeable | 12 | `census_budget.csv` |
| priced entry by entry | `N = 7,710` → `Z_local = 6.55` | `census_budget.csv` |
| priced once per axis, over the union of its ranges | `N = 3,672` → `6.44`, within 0.3% of the model space | `census_budget.csv` |
| recency | 19 of the 86 have no paper since 2019, 6 carry a published Run-3 result | `published_census.csv` |
| model independence | **29** of the 100 charged pairs carry a model-independent result, from 19 of the 74 searches | `model_independence.csv` |
| excesses a background-only sweep expects | 10 local ≥3σ and 0.0022 spurious ≥5σ over the census | `REPORTED_EXCESSES.md`, `EXCESS_COUNTING.md` |
...
| tier (i), every motivated composition once | 45 spectra, `N = 2,921` → `6.40` | `scan_summary.csv` |
| tier (ii), those compositions elsewhere | 2,366 spectra, `N = 123,231` | `scan_summary.csv` |
| tier (iii), nothing motivates it | 2,027 spectra, `N = 74,984` | `scan_summary.csv` |
| the share theory motivates | 2,411 of 4,438 spectra (54%), over 37 of the 258 fittable compositions | `scan_summary.csv`, per composition in `priority_scan.csv` |
...
| the same requirement applied to published axes | would keep 34 of 42 axes and 2,759 of 3,573 looks, so exempting them is conservative | `scan_summary.csv` |
| the two-body grid | 15 unscanned pairs cost 698 looks, from 30 (`tauH`) to 71 (`jV`); closing all of them takes `N` 3,685 → 4,382 and `Z_local` 6.44 → 6.46 | `two_body_matrix.csv` |
...
| toy validation of the reach | single stage on the full dataset 6.51 against 6.53 analytic; optimised split 7.15 against 7.09 analytic; naive 50/50 split 7.64 against 7.71 analytic (on the model space, `N = 6,597`) | `ab_split_toys.csv` |
...
|---|---|--:|--:|--:|
| mass resolution, scale | every r x2 either way | +0.107/-0.109 | +0.133/-0.191 | +0.026/-0.083 |
| mass resolution, per channel | each r independently, x2 per sigma (16-84%) | +0.026/-0.023 | +0.000/+0.000 | +0.023/-0.026 |
| mass resolution, shape | muon axes with r(M) rising to 0.10-0.20 at 3000 GeV | +0.002/-0.000 | +0.000/+0.000 | +0.000/-0.002 |
| mass resolution, prescription | worst leg / quadrature sum instead of the calibrated mean | +0.000/+0.000 | +0.000/-0.102 | +0.000/-0.102 |
| scan windows | every edge x1.4 either way (published), x1.25 (generic) | +0.031/-0.038 | +0.023/-0.024 | +0.015/-0.008 |
...
| the axis set | non-peaking axes and the dilepton overlap dropped; the 15 unscanned two-body pairs added | +0.027/-0.007 | +0.000/+0.000 | +0.007/-0.027 |
| the definition of one look | N x0.5 to x Z/sqrt(2 pi) | +0.145/-0.109 | +0.145/-0.099 | +0.009/+0.000 |
| the closed-form LEE relation | exact Gaussian-tail solution instead | +0.000/-0.036 | +0.000/-0.045 | +0.000/-0.009 |
| total | quadrature over the systematic sources above | +0.185/-0.163 | +0.231/-0.308 | +0.123/-0.232 |
