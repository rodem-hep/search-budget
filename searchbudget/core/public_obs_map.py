...
PUBLIC_OBS = {
    ...
    "N2HDM / 2HDM+S (h->aa)":        ["m(mumu) (Zd)", "m(bb)", "m(tautau)", "m(gammagamma)"],
    "Georgi-Machacek":               ["m(VV)", "m(ee) SS", "m(emu) SS", "m(mumu) SS"],
    "Singlet scalar / SM+Scalars":   ["m(HH)", "m(VV)", "m(Vgamma)"],
    "Higgs portal":                  ["m(HH)"],
    "TRSM (singlet/triplet)":        ["m(HH)", "m(VV)", "m(bb)"],
    "HeavyHiggs THDM":               ["m(tt)", "m(VV)"],
    "Vector-like quark (VLQ)":       ["m(ttZ)/m(Zt)", "m(tb)", "m(Wb)", "m(Ht)", "m(bZ)"],
    "Vector-like lepton (VLL)":      ["multilepton", "m(eZ)", "m(muZ)"],
    ...
    "Type-III seesaw":               ["multilepton", "m(eZ)", "m(muZ)"],
    ...
    "Technicolor / TC2":             ["m(VV)", "m(ee)", "m(mumu)", "m(gammajj)"],
    "Coloron / Axigluon":            ["m(jj)", "m(tt)", "m(tt)/m(jj)"],
    "Excited quark (q*/b*)":         ["m(jgamma)", "m(jj)", "m(bj)", "m(tW)", "m(bgamma)"],
    "Excited top (t*)":              ["m(tj)", "m(tgamma)"],
    "Quantum black hole":            ["m(multi)", "m(ej)", "m(muj)", "m(jgamma)",
                                      "m(emu) LFV", "m(etau) LFV", "m(mutau) LFV"],
    "RPV electroweakino (trilepton)":["m(eZ)", "m(muZ)"],
    "Excited lepton (l*)":           ["m(egamma)", "m(mugamma)", "m(taugamma)", "m(eZ)", "m(muZ)",
                                      "m(ejj)", "m(mujj)", "m(taujj)"],
    "Excited neutrino (nu*)":        ["m(ejj)", "m(mujj)"],
    "Composite Majorana N":          ["m(ejj)", "m(mujj)"],
    "Leptogluon (color-octet l)":    ["m(ej)", "m(muj)"],
    ...
    "Toponium":                      ["m(tt)"],
    "Flavoured W'/Z' (t-q)":         ["m(tj)"],
    "Color-octet scalar (MW)":       ["m(bb)"],
    "B-anomaly Z' (b-philic)":       ["m(mumu)", "m(bb)"],
    "Leptophobic Z' (dark cascade)": ["multilepton"],
    "Dark Z (mass mixing)":          ["m(ee) (Zd)", "m(mumu) (Zd)"],
    "Warped KK cascade":             ["m(3j)", "m(gammajj)", "m(VV)"],
    "MFV RPV gluino (tbs)":          ["m(tbj)"],
    "RPV resonant slepton":          ["m(mujj)", "m(mumujj)"],
...
WIDTH = {
    ...
    "Excited top (t*)":              ("narrow",    "chromomagnetic, 2-4% at f=1 like q*"),
    "Excited neutrino (nu*)":        ("narrow",    "CI decay, narrow for Lambda >= m*"),
    "Composite Majorana N":          ("narrow",    "CI width, Gamma/m << 1 for Lambda > m"),
    "Leptogluon (color-octet l)":    ("narrow",    "alpha_s m^3/Lambda^2, narrow for Lambda >~ m"),
    "Flavoured W'/Z' (t-q)":         ("narrow",    "O(0.1-1) couplings, a few %"),
    "Color-octet scalar (MW)":       ("narrow",    "MFV Yukawas, <~ a few %"),
    "B-anomaly Z' (b-philic)":       ("narrow",    "anomaly-sized g', <~ a few %"),
    "Leptophobic Z' (dark cascade)": ("narrow",    "dark-sector cascade, narrow steps"),
    "Dark Z (mass mixing)":          ("narrow",    "mixing-suppressed, <<1%"),
    "MFV RPV gluino (tbs)":          ("narrow",    "intrinsically narrow; 3-body reco sets r=0.10"),
    "RPV resonant slepton":          ("narrow",    "lambda'-suppressed, <<1%"),
    "Zee-Babu":                      ("narrow",    "<~1% for perturbative couplings"),
    ...
    "Warped KK cascade":             ("benchmark", "g_KK/W_KK a few % to ~6%; the radion narrow"),
    ...
    "Type-III seesaw":               ("narrow",    "EW width on m(lZ); counting on multilepton"),
    "Vector-like lepton (VLL)":      ("narrow",    "L doublet -> Zl EW-narrow; counting on multilepton"),
...
NSEL = {
    ...
    "m(bgamma)":     (1, "b-tagged photon+jet (no ATLAS search)"),
    "m(tgamma)":     (1, "top + photon (no ATLAS search)"),
    "m(taugamma)":   (1, "hadronic-tau + photon (no ATLAS search)"),
    "m(ejj)":        (1, "e* -> eqq system (single SR, 1906.03204)"),
    "m(mujj)":       (1, "mu + dijet (no ATLAS search)"),
    "m(taujj)":      (1, "tau* -> tau qq contact-interaction SR (2303.09444)"),
    "m(bZ)":         (1, "Z(ll) + b-tag (7/8 TeV VLQ b'/B only)"),
    "m(tj)":         (1, "t(lep) + jet in ttbar+jet (7 TeV only)"),
    "m(gammajj)":    (1, "photon + dijet (no search at any collider)"),
    "m(tbj)":        (1, "hadronic t+b+jet (no mass scan anywhere)"),
}
NSEL_DEFAULT = 2
