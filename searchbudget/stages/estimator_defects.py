...
from ..core.lee import claim_bar, p1
from ..registry import stage


@stage(
    name="estimator-defects",
    group="ab",
    summary="the published defect rate of one network scanner, and what it costs",
    outputs=["tables/estimator_defects.csv", "overviews/ESTIMATOR_DEFECTS.md"],
    needs=["tables/ab_split_scan.csv"],
)
def main(options=None):
    ...
    scan = {r["basis"]: r for r in io.read_rows(paths.table("ab_split_scan.csv"))}
    lensed = scan["lensed scan"]
    N_scan = float(lensed["N_trials"])
    k_nom = float(lensed["k_zcut3"])
    k_infl = k_nom * INFLATION_ROUND
    bar_nom, bar_infl = claim_bar(k_nom, WIDEN), claim_bar(k_infl, WIDEN)
    ...
    io.write_rows(paths.table("estimator_defects.csv"), ["quantity", "value", "source"], rows)
...
Source: {SOURCE}. Written by `searchbudget/stages/estimator_defects.py`; the two-stage numbers it compares
...
"""
    ...
    io.write_text(paths.overview("ESTIMATOR_DEFECTS.md"), md)
